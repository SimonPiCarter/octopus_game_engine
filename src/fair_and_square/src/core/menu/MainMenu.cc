...
#include "core/menu/levels/ShowcaseMenu.hh"
...
///
{
	Picture * button_l = new Picture(window_p.loadTexture("resources/button.png"), 256, 64, {1, 1 ,1}, {1, 1, 1});
	Text * text_l = new Text(&window_p, {0, 0, 0}, 0, 0);
	text_l->setText("Showcase");
	menu_l.addButton(button_l, text_l, std::bind(fas::showcaseMenu, std::ref(window_p), std::ref(quit_l)));
}

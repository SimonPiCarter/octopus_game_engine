...
void arenaMenu(cuttlefish::Window &window_p, bool &exit_p)
{
...

{
	...
	menu_l.addButton(button_l, text_l, std::bind(fas::runArena, std::ref(window_p), 50));
}
{
	Picture * button_l = new Picture(window_p.loadTexture("resources/button.png"), 256, 64, {1, 1 ,1}, {1, 1, 1});
	Text * text_l = new Text(&window_p, {0, 0, 0}, 0, 0);
	text_l->setText("Play Arena Level (500)");
	menu_l.addButton(button_l, text_l, std::bind(fas::runArena, std::ref(window_p), 500));
}
{
	Picture * button_l = new Picture(window_p.loadTexture("resources/button.png"), 256, 64, {1, 1 ,1}, {1, 1, 1});
	Text * text_l = new Text(&window_p, {0, 0, 0}, 0, 0);
	text_l->setText("Replay Arena Level");
	menu_l.addButton(button_l, text_l, std::bind(fas::replayArena, std::ref(window_p)));
}
{
	Picture * button_l = new Picture(window_p.loadTexture("resources/button.png"), 256, 64, {1, 1 ,1}, {1, 1, 1});
	Text * text_l = new Text(&window_p, {0, 0, 0}, 0, 0);
	text_l->setText("Load Arena Level");
	menu_l.addButton(button_l, text_l, std::bind(fas::loadArena, std::ref(window_p)));
}
{
	Picture * button_l = new Picture(window_p.loadTexture("resources/button.png"), 256, 64, {1, 1 ,1}, {1, 1, 1});
	Text * text_l = new Text(&window_p, {0, 0, 0}, 0, 0);

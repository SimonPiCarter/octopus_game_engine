...
void CommandSetPositionFromStep::revert(State &state_p) const
{
	...
	CommandDataWithData<AttackMoveData> *data_l = dynamic_cast<CommandDataWithData<AttackMoveData> *>(ent_l->getFrontQueue()._data);
	if(data_l->_data._positionFromAttack != _newPos)
	{
		throw std::logic_error("Setting positionFrom had incoherent old position regarding content in state");
	}
	data_l->_data._positionFromAttack = _oldPos;
}

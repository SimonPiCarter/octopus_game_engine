...
void NoModifier::newAttackSteppable(std::vector<Steppable *> &vec_r, const Entity &ent_p, const Entity &target_p,
...
void CompositeModifier::newAttackSteppable(std::vector<Steppable *> &vec_r, const Entity &ent_p, const Entity &target_p,
	State const &state_p, CommandContext const &commandContext_p, Step const &step_p) const
{
	for(CompositableModifier const &mod_l : _modifiers)
	{
		...
		std::visit([&](auto&& arg)
		{
			arg.newAttackSteppable(vec_r, ent_p, target_p, state_p, commandContext_p, step_p, true);
		}, mod_l);
	}
}
	...
	State const &state_p, CommandContext const &commandContext_p, Step const &step_p)
{
	...
	std::visit([&](auto&& arg)
	{
		arg.newAttackSteppable(vec_r, ent_p, target_p, state_p, commandContext_p, step_p);
	}, ent_p._attackMod);
}

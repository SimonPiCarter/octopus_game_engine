...
TEST(buffStepTest, simple_damage_return)
{
	octopus::EntityModel unitModel_l { false, 1., 1., 10. };
	...
	buff_l._coef = 0.;
	buff_l._offset = 1.;
	buff_l._type = TyppedBuff::Type::DamageReturn;
	...
	EXPECT_NEAR(0., to_double(state_l->getEntity(Handle(0))->getDamageReturn()), 1e-5);
	...
	// buff has been applied
	EXPECT_NEAR(1., to_double(state_l->getEntity(Handle(0))->getDamageReturn()), 1e-5);

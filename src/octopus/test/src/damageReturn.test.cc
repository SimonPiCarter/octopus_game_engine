...
TEST(damageReturnTest, simple)
{
	octopus::EntityModel unitModel_l { false, 1., 1., 10. };
	unitModel_l._damageReturn = 1.;
	...
	EXPECT_NEAR(2., to_double(state_l->getEntity(Handle(0))->_hp), 1e-5);
}

TEST(damageReturnTest, simple_bonus_dmg)
{
	octopus::EntityModel unitModel_l { false, 1., 1., 10. };
